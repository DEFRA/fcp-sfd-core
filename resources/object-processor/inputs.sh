...
## Entra ID auth (AUTH_ENTRA_ENABLED) is on by default; Cognito auth (AUTH_COGNITO_ENABLED) is off by default.
## Add the token to each authenticated request: -H "Authorization: Bearer ${TOKEN}"
## Cognito token instructions are on Confluence: https://eaflood.atlassian.net/wiki/spaces/SFD/pages/6468732912/Generate+Cognito+Bearer+token+for+use+with+Object-Processor
